...
Whereas the numeric :mod:`qcr_repro.compute_graph` identifies nodes by hashing
rounded complex matrices (tolerance 1e-5 implicit, phase-normalization
dependent), the graphs here identify nodes by the exact *signed* symplectic
tableau of :mod:`qcr_repro.symplectic`.  For Clifford pools this is a
bijective invariant of unitaries up to global phase, giving:

  * bit-exact, tolerance-free node identity and circuit equivalence,
  * integer-only graph construction (no floating point in the hot loop),
  * provably correct replacement: a returned word implements a unitary equal
    to the block up to global phase *by construction*,
  * compact integer node keys (each 2n-row tableau packed into one int).

In addition, each node stores a bounded set of Pareto-optimal factorizations
indexed by (two-qubit count, length), so lookups can minimize a hardware-cost
objective (e.g. the number of RXX/CZ gates) rather than length alone -- the
cost-aware direction explicitly deferred to future work in Rosenhahn et al.
"""
