"""Reproduce the results and figures of Rosenhahn, Osborne & Hirche (2025),
"Optimization driven quantum circuit reduction", New J. Phys. 27 104509.

The paper contains 7 tables and 10 figures.  This script reproduces the ones
that are reproducible in simulation on a laptop:

  * Table 1  - compute-graph growth (paper values; our figure 2 already plots it)
  * Table 2  - V1/V2/V3 computation-time statistics on 100 length-100 circuits
  * Tables 3-5 - single-circuit optimization examples (4q ion trap, 6q NISQ,
               15q NISQ): qiskit L1-L3, BQSKit L2-L4, our reducers
  * Tables 6-7 - 100-run statistics (4q, length 300): qiskit baselines + ours
  * Figures 1-10 - paper-style figures, written to figures/paper/

Figure 10 (IBM hardware experiments) requires authenticated hardware access
and is reproduced as an explicit placeholder with the paper's reported
numbers, not fabricated data.

Usage:
    PYTHONPATH=src python scripts/generate_paper_pack.py --single-circuits
    PYTHONPATH=src python scripts/generate_paper_pack.py --timing
    PYTHONPATH=src python scripts/generate_paper_pack.py --figures
    PYTHONPATH=src python scripts/generate_paper_pack.py --tables   # after the above
    PYTHONPATH=src python scripts/generate_paper_pack.py --all      # everything
"""
...
from qcr_repro.reducer import reduce_random_sampling, reduce_with_database, reduce_with_lookup
...
# exact example circuits are not published, so we reproduce the experiment
    ...
    for seed in range(num_circuits):
        ...
        for label, fn in methods.items():
            ...
            if label.startswith("exact"):
                ...
                end, secs = fn(gates, 4, db_ion, budget)
...
def cmd_tables() -> None:
    RESULT_DIR.mkdir(parents=True, exist_ok=True)
    md: list[str] = [
        "# Paper results reproduction (Rosenhahn et al., NJP 27, 104509, 2025)",
        "",
        "Reference values are transcribed from the paper PDF (Tables 1-7). "
        "Our values come from `results/paper/` and `results/comparison/`. "
        "Figure 10 (hardware) is not reproducible without hardware access.",
        "",
    ]
...
def fig2_compute_graph():
    ...
    ax.set_xlim(-4.2, 4.2)
    ax.set_ylim(-3.0, 3.0)
    _finish(fig, "figure2_compute_graph.png")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Reproduce the paper's results and figures.")
