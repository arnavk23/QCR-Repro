"""Correctness checks for the batched sweep and the pre-passes.

1. ``batched_sweep`` must be *bit-identical* to the scalar ``_sweep_reduce``:
   same final gate list and same replacement count, on random circuits of both
   gate sets.
2. The algebraic / ZX pre-passes must preserve the circuit unitary (up to a
   global phase), and must produce only pool-representable gates (every output
   gate must exist in the discrete token pool).
3. ``reduce_circuit`` with ``use_batched=True`` must never be *worse* than the
   scalar reducer under the same seed and time budget, and must preserve the
   input unitary.  (The two trajectories are not bit-comparable because the
   time budget is hit at different loop iterations -- the batched sweep is
   faster -- which advances the RNG stream differently; only the sweep itself
   is bit-identical, as verified in check 1.)

Usage:
    PYTHONPATH=src python scripts/check_batched_matches_scalar.py
"""
...
from qcr_repro.tokenizer import TokenPool
...
def check_batched_matches_scalar(db_ion, db_nisq) -> int:
    failures = 0
...
def main() -> None:
    ...
    failures += check_batched_matches_scalar(db_ion, db_nisq)
